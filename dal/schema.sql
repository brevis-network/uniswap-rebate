...
-- routers configured in RouterBeneficiaryRegistry on Unichain.
-- chid is source chain id of swap logs.
CREATE TABLE IF NOT EXISTS claimer (
    chid BIGINT NOT NULL, -- chainid
    router TEXT NOT NULL,  -- 0x... router address
    beneficiary TEXT NOT NULL, -- 0x... beneficiary address set in registry
    fetch_blk BIGINT NOT NULL DEFAULT 0, -- last processed block on source chain for this router
...
CREATE TABLE IF NOT EXISTS reqs (
    ...
    usr_req JSONB, -- webapi.NewProofReq from user. nullable for scheduled backend jobs
